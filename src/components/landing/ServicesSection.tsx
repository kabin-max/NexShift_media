...
export default function ServicesSection() {
  return (
    <section className="w-full bg-black py-24 md:py-32 overflow-hidden flex flex-col gap-12 border-t border-zinc-900/50 relative">
            ...
            return (
              <Link key={service.id} href={`/services?category=${encodeURIComponent(service.title)}`} className="block w-full">
                <motion.div
                  ...
                  className="group flex flex-col md:grid md:grid-cols-12 gap-6 md:gap-4 py-10 md:py-12 border-b border-zinc-800/80 items-center transition-colors duration-500 hover:bg-white/[0.01] px-4 -mx-4 rounded-lg relative overflow-hidden cursor-pointer"
                >
                  ...
                  <div className="col-span-1 flex justify-start items-center">
                    <span
                      className="text-xs md:text-sm font-mono text-zinc-500 border border-zinc-800 px-2.5 py-1 rounded-md group-hover:text-white group-hover:border-zinc-700 transition-all duration-350"
                    >
                      {service.num}
                    </span>
                  </div>
                  ...
                  <div className="col-span-3 flex justify-start items-center">
                    <h3
                      className="text-2xl md:text-3xl lg:text-4xl font-bold text-zinc-150 tracking-tight font-sans transition-transform duration-350 group-hover:translate-x-2"
                      style={{ fontFamily: "var(--font-geist-sans), sans-serif" }}
                    >
                      {service.title}
                    </h3>
                  ...
                  <div className="col-span-2 flex flex-col items-start gap-1">
                    {service.tags.map((tag, tIdx) => (
                      <span
                        key={tIdx}
                        className="text-[10px] md:text-xs font-semibold tracking-wider text-rose-500/90 uppercase font-sans"
                        style={{ fontFamily: "var(--font-geist-sans), sans-serif" }}
                      >
                        {tag}
                      </span>
                  ...
                  <div className="col-span-3 flex items-center">
                    <p
                      className="text-zinc-400 group-hover:text-zinc-200 text-sm md:text-base leading-relaxed transition-colors duration-350 font-sans"
                      style={{ fontFamily: "var(--font-geist-sans), sans-serif" }}
                    >
                      {service.description}
                    </p>
                  ...
                  <div className="col-span-3 w-full flex justify-between md:justify-end items-center gap-4">
                    <div className="relative overflow-hidden rounded-xl aspect-[4/3] w-[118px] md:w-[162px] border border-zinc-800/80 shadow-md flex-shrink-0">
                      <img
                        src={service.image}
                        alt={service.title}
                    ...
                    <div className="p-2 rounded-full border border-zinc-800 text-zinc-500 group-hover:text-white group-hover:border-zinc-500 transition-all duration-350 flex-shrink-0">
